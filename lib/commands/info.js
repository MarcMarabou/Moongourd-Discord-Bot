...
module.exports = {
    ...
    callback: function (client, message, args) {
        if (args.length < 4) return message.channel.send('Please list: server charactername dungeon-acronym boss');
        ...
        if (!MOONGOURD_AREA.hasOwnProperty(args[2])) return message.channel.send('Dungeon acronym not recognized. Please try again');

        let bossNumber;
        if (['1', 'first', 'ph2', 'f1',].includes(args[3].toLowerCase())) bossNumber = 1;
        ...
        else return message.channel.send('4th argument must be boss-number, phase or floor');

        Object.assign(postData, {
            playerServer: args[0][0].toUpperCase() + args[0].toLowerCase().slice(1),
            playerName: args[1],
            areaId: MOONGOURD_AREA[args[2]][0],
            bossId: MOONGOURD_AREA[args[2]][bossNumber].id
        });
        ...
        request({
            ...
            const avg = body[0][0].avg;
            const max = body[0][0].max;
            ...
            utils.formatEmbeddedPost(thumbnail, color, author, fields);
        });
    }
}
